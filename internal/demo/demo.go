// Package demo — история выдуманного интернет-магазина за две недели:
// частые, новые и одна решённая проблема, разные браузеры, версии и
// пользователи. Её заливают в демо в браузере (cmd/snag-wasm) и в
// настоящий Snag (snag-bench -demo), чтобы сразу было что посмотреть.
package demo
...
// ResolvedTitle — проблему с таким началом заголовка стоит пометить
// решённой после заливки: так в демо видна вкладка «Решённые».
const ResolvedTitle = "SyntaxError"

// Envelopes возвращает конверты истории: события размазаны по двум неделям
// до now, часть — за последние сутки, чтобы графики были живыми.
func Envelopes(now time.Time) []string {
	...
	for i, is := range issues {
		for n := 0; n < is.count; n++ {
			...
			out = append(out, `{"event_id":"`+ev["event_id"].(string)+`"}`+"\n"+`{"type":"event"}`+"\n"+string(body)+"\n")
		}
	}
	return out
}
